...
- `src/game/dtii-wall-insets.ts` — GENERATED (`npm run assets:insets`): per-blob-cell opaque-art bounding insets, measured from walls_high.png. Colliders derive from these so walls collide exactly where they are VISIBLE — the art's painted ground margins (side-bar insets, stub flanks) stay walkable. Feel result: uniform slight sprite-tuck against every wall direction (verified numerically: body stops at mod-16 offsets 11/5/0/0 for W/E/N/S).
